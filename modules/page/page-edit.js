...
$(document).ready(function() {
	...
	// Change between responsive modes on click
	// 1. Default (No container)
	// 2. Fluid container, centered
	// 3. Fluid container, left
	elem = $('<img src="'+$.glue.base_url+'modules/page/responsive-temp-icon.png" width="32" height="32">');
  ...
  $(elem).bind('mousedown', function (e) {
    ...
    $.glue.backend({ method: 'page.set_responsive_mode', 'mode': modes[nextMode], 'page': $.glue.page });
  ...
  // Drag to set container width
 	elem = $('<img src="'+$.glue.base_url+'modules/page/page-container-size.png" width="32" height="32">');
  $(elem).attr('title', 'change responsive mode (' + (parseInt(document.documentElement.style.getPropertyValue('--container-width')) || 'unset') + ')');
  $(elem).bind('mousedown', function (e) {
    var that = this;
    let initialWidth = parseInt(document.documentElement.style.getPropertyValue('--container-width'));
    $.glue.slider(e, function (x, y, evt) {
      var containerWidth = Math.max(200, initialWidth + x);
      document.documentElement.style.setProperty('--container-width', containerWidth)
    }, function (x, y) {
   			$.glue.backend({ method: 'page.set_container_width', 'width': (initialWidth + parseInt(x)), 'page': $.glue.page });
